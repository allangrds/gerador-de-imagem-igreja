...
function setElementValues (content) {
  document.getElementsByClassName('message')[0].innerHTML = content['preacher-message-title']
  document.getElementsByClassName('preacher-name')[0].innerHTML = content['preacher-name']

  if (content['preacher-image-url']) {
    document.getElementsByClassName('preacher-image')[0].src = content['preacher-image-url']
  } else if (content['preacher-image-input'] && content['preacher-image-input'][0]) {
    const reader = new FileReader()
    reader.onload = function () {
      document.getElementsByClassName('preacher-image')[0].src = reader.result
    }
...
function saveThumbnailYoutubeImage () {
  const div = document.getElementsByClassName('new-thumb')[0]
